import { PrismaClient, FreshnessStatus, DataSourceKind } from "@prisma/client";
...
async function main() {
  for (const source of ingestionSources) {
    await prisma.dataSource.upsert({
      ...
      update: { name: source.name, owner: source.owner, url: source.url, license: source.license, refreshCadence: source.refreshCadence },
      create: {
        key: source.key,
        name: source.name,
        owner: source.owner,
        ...
        kind: source.productionUse === "official" ? DataSourceKind.OFFICIAL : source.productionUse === "licensed" ? DataSourceKind.LICENSED : source.productionUse === "partner" ? DataSourceKind.PARTNER : DataSourceKind.DEMO
      }
    });
  }
  ...
  for (const medicine of medicines) {
    const created = await prisma.medicine.upsert({
      ...
      create: {
        ...
        searchAliases: { create: [medicine.brandName, medicine.genericName, ...medicine.activeIngredients, ...(medicine.commonMisspellings || [])].map((alias) => ({ alias, aliasType: "normalized", confidence: 0.85 })) },
  ...
  for (const plan of plans) {
    const created = await prisma.insurancePlan.upsert({
      ...
      create: {
        ...
        estimatedAnnualDrugCost: plan.estimatedAnnualDrugCost,
    ...
    for (const [medicineSlug, rule] of Object.entries(plan.formulary)) {
      ...
      await prisma.formularyRule.upsert({
        where: { planId_medicineId: { planId: created.id, medicineId: medicine.id } },
        update: {
          tier: rule.tier,
          copay: rule.copay,
