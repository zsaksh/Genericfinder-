  ...
  return (
    <article className="relative overflow-hidden rounded-organic border border-ink/10 bg-paper p-5 shadow-card">
      ...
      <div className="mt-5 flex flex-col gap-2 sm:flex-row">
        <Button asChild>
          <a href={`/out/${plan.slug}${medicineSlug ? `?medicine=${medicineSlug}` : ""}`} rel="nofollow sponsored" onClick={() => trackEvent({ event: "click_affiliate", entitySlug: plan.slug, metadata: { medicineSlug } })}>View offer <ArrowUpRight className="h-4 w-4" /></a>
        </Button>
