...
<SourcePanel medicine={medicine} />
      ...
      <div className="mt-14"><RecommendationPanel medicineSlug={medicine.slug} /></div>
      <section className="mt-14"><h2 className="font-serif text-4xl font-semibold">Insurance plans that model coverage</h2><p className="mt-3 max-w-3xl text-graphite">Plan cards show modeled tier and cost details plus affiliate disclosure. Verify county, plan year, network, and formulary directly with the insurer before enrolling.</p><div className="mt-5 grid gap-5 lg:grid-cols-3">{coverage.map(({ plan }) => <PlanCard key={plan.slug} plan={plan} medicineSlug={medicine.slug} />)}</div><div className="mt-8 overflow-x-auto"><CoverageMatrix medicineSlug={medicine.slug} /></div></section>
